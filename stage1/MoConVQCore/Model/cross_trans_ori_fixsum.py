...
class Text2Motion_Transformer(nn.Module):
    ...
    def __init__(self, 
        ...
        # 8, 4, 3
        # -> 8, 2, 2
        self.trans_temporal = CrossCondTransFeature(num_vq, embed_dim, clip_dim, block_size, 12, n_head, drop_out_rate, fc_rate)
        ...
        self.trans_base = CrossCondTransBase(num_vq, embed_dim, embed_dim, block_size, 4, n_head, drop_out_rate, fc_rate)
        self.trans_head = CrossCondTransHead(num_vq, embed_dim, block_size, 1, n_head, drop_out_rate, fc_rate)
...
<<<<<<< HEAD:stage1/MoConVQCore/Model/cross_trans_ori_fixsum.py
...
>>>>>>> origin/main:MoConVQCore/Model/cross_trans_ori_fixsum.py
        ...
        for k in range(max_length):
            ...
            fs = self.trans_temporal(latent, clip_feature, bert_feature, bert_mask)
            cur_fs = fs[:, -1, :]
            ...
            # print(ls.shape)
        # print(torch.cat(idxs, dim=1).view(-1, 4))
...
class CrossCondTransFeature(nn.Module):
    ...
    def __init__(self, 
        ...
        for i in range(num_layers):
            self.blocks.append(CrossBlock(embed_dim, block_size, n_head, drop_out_rate, fc_rate, bert_dim, bert_max_len))
...
class CrossCondTransHead(nn.Module):
    ...
    def __init__(self, 
        ...
        self.head = nn.ModuleList( [nn.Linear(embed_dim, num_vq + 1, bias=False) for i in range(8)]  )
