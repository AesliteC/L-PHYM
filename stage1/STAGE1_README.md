...
这说明显式 segment 边界是更严格的训练/推理一致性修复，并且真实段长比例能让
finetuned 在 R-precision 和 matching score 上超过 baseline；但 FID 基本打平略差，
early-stop 也变差。因此当前 checkpoint 还不能声称在 stricter explicit-boundary
protocol 下全面超过 baseline。最终报告应同时写出这点，不能只拿 plain-`" then "`
的最好数值作为完整胜利。
