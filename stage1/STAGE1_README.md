...
`generate_long_motion.py --segments-json`，用于严格按训练 clip 边界推理；旧两列 TSV
和默认 `" then "` 行为保持兼容。

当前推荐的 metric-balanced checkpoint：
...
paper metrics 更混合；`checkpoint_epoch_3.pth` 是当前更适合作为报告主结果的
...
这说明显式 segment 边界是更严格的训练/推理一致性修复，并且 finetuned 仍改善
时长覆盖和早停；但当前 checkpoint 在 stricter explicit-boundary protocol 下还没有
超过 baseline 的 approximate FID/matching。最终报告应同时写出这点，不能只拿
plain-`" then "` 的最好数值作为完整胜利。
...
```text
sample_name<TAB>long prompt text<TAB>["segment caption 1", "segment caption 2"]
```
...
当前推荐的 Stage1 engineering/metric 结果是 2026-06-14 artifact label 的 segment-aligned native-retarget run：73 train / 18 val long sequences 形成 476 train windows 和 117 val windows，head-only 5 epoch 训练中 val loss 从 16.863 降到 15.675。Held-out Val8 top-p baseline-vs-finetuned 对比中，plain-`" then "` protocol 下当前更推荐 epoch3 checkpoint：平均长度从 1296 frames 提升到 1308 frames，早停率从 0.375 降到 0.25，root path 从 2.229 提升到 2.374。Approximate T2M evaluator route 上，FID 从 18.136 降到 16.209，R-precision@1 从 0.25 提升到 0.375，R-precision@2 持平 0.625，matching score 从 4.322 降到 4.189；R-precision@3 仍从 1.00 降到 0.625，lag20 repeat fraction 也从 0.0064 增到 0.0105。

因此当前结论是：长 HumanML3D 合成可用；旧 hand-written HumanML3D-to-state cache 不适合正式结论；BVH-to-character native retarget 是目前真正 work 的主线；训练和推理必须使用一致的 segment 语义。默认 `" then "` 分段能给出当前最好 partial metric improvement，但对含有句内 `then` 的 HumanML3D caption 会误拆；新增的显式 segment JSON 路径能严格匹配训练 clip 边界，在 Val8 上改善时长/早停但尚未改善 approximate FID/matching。因此保守 head-only GPT 微调已经取得部分但不完整的 baseline 改进，后续真正要超过 baseline 的全部 paper-style 指标，还需要更好的 segment-level training objective、更多 native-retarget 数据或重新选择/训练 checkpoint。论文指标相关资产已经具备，但 MoConVQ BVH 到 HumanML3D 263-d feature 的近似 adapter 带有骨架适配误差，且 evaluator 会截断长序列，因此当前 FID/R-precision 必须标为 approximate evaluator-adapter route，不能替代原生 HumanML3D/SMPL 无偏评估。
