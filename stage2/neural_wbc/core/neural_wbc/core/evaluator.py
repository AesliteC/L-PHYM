...
class Evaluator:
    ...
    def __init__(
        ...
        self._failure_reasons: list[str | None] = [None] * self._num_unique_ref_motions
    ...
    def _update_failure_metrics(self, newly_terminated: torch.Tensor, info: dict):
        ...
        failure_condition_items = [
            (k, v[:counted_envs].flatten())
            for k, v in info["termination_conditions"].items()
            if k != "reference_motion_length"
        ]
        if len(failure_condition_items) == 0:
            self._metrics.success_rate = 1 - torch.sum(self._failed).item() / end_id
            return
        failed_conditions = torch.stack([v for _, v in failure_condition_items])

        # Update failed environments
        newly_failed = torch.logical_and(newly_terminated[:counted_envs], torch.any(failed_conditions, dim=0))
        self._failed[start_id:end_id] |= newly_failed
        for local_id in torch.nonzero(newly_failed).flatten().tolist():
            reasons = [name for name, values in failure_condition_items if bool(values[local_id].item())]
            self._failure_reasons[start_id + local_id] = ",".join(reasons) if reasons else "unknown"
        self._metrics.success_rate = 1 - torch.sum(self._failed).item() / end_id
    ...
    def conclude(self):
        ...
        self._metrics.print()
        self._print_successful_motions()
        if self._metrics_path:
            self._metrics.save(self._metrics_path)
            self._save_per_motion_results(self._metrics_path)

    def _get_motion_keys(self) -> list[str]:
        """Returns reference motion keys in dataset order."""
        motion_lib = self._ref_motion_mgr.motion_lib
        motion_keys = getattr(motion_lib, "_motion_data_keys", None)
        if motion_keys is None:
            return [str(i) for i in range(self._num_unique_ref_motions)]
        if hasattr(motion_keys, "tolist"):
            motion_keys = motion_keys.tolist()
        return [str(key) for key in motion_keys]

    def _get_per_motion_results(self) -> list[dict]:
        """Returns per-motion success/failure records."""
        motion_keys = self._get_motion_keys()
        failed = self._failed.detach().cpu().tolist()
        return [
            {
                "motion_idx": motion_idx,
                "motion_key": motion_keys[motion_idx] if motion_idx < len(motion_keys) else str(motion_idx),
                "success": not bool(failed[motion_idx]),
                "failure_reason": self._failure_reasons[motion_idx],
            }
            for motion_idx in range(self._num_unique_ref_motions)
        ]

    def _print_successful_motions(self):
        """Print successful motion indices and keys for replay/recording."""
        successful = [record for record in self._get_per_motion_results() if record["success"]]
        print(f"Successful motions ({len(successful)}):")
        for record in successful:
            print(f"  {record['motion_idx']}: {record['motion_key']}")

    def _save_per_motion_results(self, directory: str):
        """Saves per-motion success/failure records for later replay."""
        file_dir = Path(directory)
        file_dir.mkdir(parents=True, exist_ok=True)
        output_path = file_dir / "per_motion_results.json"
        content = {
            "num_motions": self._num_unique_ref_motions,
            "success_count": int(self._num_unique_ref_motions - torch.sum(self._failed).item()),
            "failure_count": int(torch.sum(self._failed).item()),
            "motions": self._get_per_motion_results(),
        }
        with open(output_path, "w") as fh:
            json.dump(content, fh, indent=2)
        print(f"Per-motion results saved to: {output_path}")
