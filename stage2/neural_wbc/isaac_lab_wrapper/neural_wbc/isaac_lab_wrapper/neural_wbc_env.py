...
class NeuralWBCEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: NeuralWBCEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Visualizations
        self._ref_motion_visualizer = RefMotionVisualizer() if self.cfg.visualize_ref_motion else None
    ...
    def _setup_scene(self):
        ...
        if self.cfg.recording_lighting:
            self._spawn_recording_floor_grid()
        ...
        if self.cfg.recording_lighting:
            ...
            dome_light_cfg.func("/World/Light/Dome", dome_light_cfg)

            key_light_cfg = sim_utils.SphereLightCfg(
                intensity=105000.0,
                color=(1.0, 0.88, 0.74),
                radius=1.0,
            )
            key_light_cfg.func("/World/Light/Key", key_light_cfg, translation=(1.2, -2.0, 4.0))

            fill_light_cfg = sim_utils.SphereLightCfg(
                intensity=3500.0,
                color=(0.45, 0.58, 1.0),
                radius=2.0,
            )
            fill_light_cfg.func("/World/Light/Fill", fill_light_cfg, translation=(-2.8, 2.8, 2.0))

            rim_light_cfg = sim_utils.SphereLightCfg(
                intensity=18000.0,
                color=(0.80, 0.88, 1.0),
                radius=0.8,
            )
            rim_light_cfg.func("/World/Light/Rim", rim_light_cfg, translation=(-2.0, 2.8, 3.0))
        else:
            light_cfg = sim_utils.DomeLightCfg(intensity=2000.0, color=(0.75, 0.75, 0.75))
            light_cfg.func("/World/Light", light_cfg)

    def _spawn_recording_floor_grid(self):
        grid_extent = 10.0
        grid_spacing = 0.5
        line_width = 0.012
        line_height = 0.002
        z_offset = 0.003
        grid_material = sim_utils.PreviewSurfaceCfg(
            diffuse_color=(0.11, 0.12, 0.13),
            roughness=0.9,
        )
        line_count = int(grid_extent / grid_spacing)

        for idx in range(-line_count, line_count + 1):
            coord = idx * grid_spacing
            x_line_cfg = sim_utils.MeshCuboidCfg(
                size=(line_width, 2.0 * grid_extent, line_height),
                visual_material=grid_material,
            )
            x_line_cfg.func(
                f"/World/RecordingGrid/x_{idx + line_count:02d}",
                x_line_cfg,
                translation=(coord, 0.0, z_offset),
            )
            y_line_cfg = sim_utils.MeshCuboidCfg(
                size=(2.0 * grid_extent, line_width, line_height),
                visual_material=grid_material,
            )
            y_line_cfg.func(
                f"/World/RecordingGrid/y_{idx + line_count:02d}",
                y_line_cfg,
                translation=(0.0, coord, z_offset),
            )
    ...
    def _get_observations(self) -> dict:
        ...
        if self._ref_motion_visualizer is not None:
