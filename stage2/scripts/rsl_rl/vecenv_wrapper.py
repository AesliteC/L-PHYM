...
class RslRlNeuralWBCVecEnvWrapper(EnvironmentWrapper):
    ...
    def __init__(self, env: NeuralWBCEnv):
        ...
        super().__init__(mode=base_env.cfg.mode)

        # initialize the wrapper
        self._env = env
        self._base_env = base_env
        # store information required by wrapper
        self.num_envs = self._base_env.num_envs
        self.device = self._base_env.device
        self.reference_motion_manager = self._base_env.reference_motion_manager
        self.max_episode_length = self._base_env.max_episode_length
        if hasattr(self._base_env, "action_manager"):
            self.num_actions = self.unwrapped.action_manager.total_action_dim
        else:
            self.num_actions = self._base_env.action_space.shape[1]
        if hasattr(self._base_env, "observation_manager"):
            self.num_obs = self._base_env.observation_manager.group_obs_dim["teacher_policy"][0]
        else:
            self.num_obs = self._base_env.num_observations
        # -- privileged observations
        if hasattr(self._base_env, "observation_manager") and "critic" in self._base_env.observation_manager.group_obs_dim:
            self.num_privileged_obs = self._base_env.observation_manager.group_obs_dim["critic"][0]
        elif hasattr(self._base_env, "state_space"):
            self.num_privileged_obs = self._base_env.state_space.shape[1]
        else:
